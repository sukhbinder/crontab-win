...
def test_parse_line_days_range():
    ...
    line26_05 = (
        '0 0 26-05 9 * cmd.exe /c "echo test..."'  # days range spans the next month
    )
...
def test_parse_line_month_range():
    ...
    line11_02 = (
        '0 0 01 11-02 * cmd.exe /c "echo test..."'  # months range spans the next year
    )
...
def test_parse_line_name_dow():
    ...
    line_wed_sat_tue = (
        '0 0 01 9 wed,sat-tue cmd.exe /c "echo test..."'  # range spans the next period
    )
...
def test_parse_line_name_month():
    ...
    line_janfebmar_aug_oct_feb = (
        '0 0 01 aug,oct-feb * cmd.exe /c "echo test..."'  # range spans the next period
    )
    ...
    with pytest.raises(ValueError):
        mins, hour, day, month, dow, command = app.parse_line(
            line_janfebmar_aug_oct_feb
        )
